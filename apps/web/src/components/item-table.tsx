  ...
  return (
    <div className='border-default-border px-sm h-12 border-b'>
      <div
        ...
        className='gap-x-xs px-md text-gray-7 dark:text-dark-0 grid h-full items-center font-semibold'
      >
        {children}
      </div>
    </div>
...
function DataRow({ children }: PropsWithChildren) {
  ...
  return (
    <div
      style={{ gridTemplateColumns: context?.gridTemplateColumns }}
      className='gap-x-xs border-default-border bg-gray-1 px-md py-xs focus-within:outline-mantine-default dark:bg-dark-5 grid items-center rounded-sm border text-sm'
    >
      {children}
    </div>
  );
